/**
 * Starburst + Median Radius  (PupilLens original, v7)
 * =====================================================
 * 24-ray starburst from MediaPipe iris centre; Canny edges; radial gradient
 * filter; median radius as the estimate.  Robust to ≤50% outlier rays.
 * Quality = MAD/r (lower = better).  Drops frame if quality > 0.20.
 */
(function(){
'use strict';
...
window.PupilAlgos['starburst'] = {
  ...
  run(inp) {
    ...
    if (quality > QUALITY_MAX || r < innerR || r > outerR) return fail();
    ...
    hitX.forEach((hx,i)=>{
      ...
      const idx=(hy*w+hx)*4; dbg[idx]=255;dbg[idx+1]=230;dbg[idx+2]=0;dbg[idx+3]=255;
    });
